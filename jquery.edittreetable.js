    ...
    $.fn.bstreetable = function (options) {
        ...
        function generateColumn(row, extfield) {
            ...
            switch (extfield.type) {
                case "input":
                    generatedCol = $("<input type='text' class='form-control input-sm'/>").val(row[extfield.key]).attr("data-oldval", row[extfield.key]).attr("name", extfield.key);
                    break;
                default:
        ...
        function generateTreeNode(curElement, row, curLevel, isPrepend) {
            ...
            if (row.innercode) { // if the node is category
                if (curLevel - 0 >= settings.maxlevel) {
                    $('<li class="j-expend"></li>').append('<label class="fa p-xs"></label>').append($("<input type='text' class='form-control input-sm' readonly required/>").attr("data-oldval", row['name']).val(row['name']).attr("name", "name")).attr('data-id', row.id).appendTo(dom_ul);
                    dom_ul.attr("data-loaded", true);
                } else {
                    $('<li class="j-expend"></li>').append('<label class="fa fa-plus p-xs"></label>').append($("<input type='text' class='form-control input-sm' readonly required/>").attr("data-oldval", row['name']).val(row['name']).attr("name", "name")).attr('data-id', row.id).appendTo(dom_ul);
                }
            } else { // if the node is filter
                if (curLevel - 0 >= settings.maxlevel) {
                    switch (filterElement.type) {
                        case "checkbox":
                            ...
                            $('<li class="j-expend"></li>').append('<label class="fa fa-filter filter-space"></label>').append($("<div> class='form-control input-sm' readonly required>" + filterElement.label + "[" + filterElement.type + "]</div>").attr("data-oldval", row['name']).val(row['name']).attr("name", "name")).attr('data-id', "filter" + filterIndex).append(ulValue).appendTo(dom_ul);
                            dom_ul.attr("data-loaded", true);
                            break;
                        case "radio":
                            var ulValue = $('<ul class="filter-value"></ul>')
